...
### 🚀 Live Working Demo (Cloudflare Tunnel)
- **URL:** [https://score-publications-athletics-auction.trycloudflare.com](https://score-publications-athletics-auction.trycloudflare.com)
- **Admin Login:** `admin@gmail.com` / `admin@1234`

This single-origin deployment serves both the React production build and Spring Boot API through a zero-dependency reverse-proxy on port `5050` exposed via a Cloudflare Quick Tunnel.
...
## 🚀 One-Click Live Quick Deployment (Single-Origin Cloudflare Tunnel)

To serve both the React frontend and Spring Boot backend from a single origin on one public URL without CORS configuration issues, we utilize a reverse proxy running on port `5050` and expose it via Cloudflare Quick Tunnels.

### How to Run the Deployment Flow Locally:

#### 1. Build the React Frontend (Same-Origin)
Navigate to the frontend folder and build the UI assets with a relative API URL:
```bash
cd frontend
...
Ensure your database/H2 configuration is set, then run the Spring Boot API (runs on port `9090` by default):
```bash
cd eCommersApp
.\mvnw spring-boot:run
```

#### 3. Start the Single-Origin Proxy Server
Start the lightweight Node.js proxy server from the `deploy` folder. It serves the production React build and routes `/ecom/*` API requests directly to the Spring Boot backend:
```bash
cd deploy
node proxy-server.js
```

#### 4. Expose Live via Cloudflare Quick Tunnel
In the `deploy` folder, run the pre-downloaded Cloudflare binary to generate a public HTTPS URL:
```bash
.\cloudflared.exe tunnel --url http://localhost:5050 --no-autoupdate --logfile app-tunnel.log
```
Check the generated `app-tunnel.log` file to find your live `.trycloudflare.com` link.
