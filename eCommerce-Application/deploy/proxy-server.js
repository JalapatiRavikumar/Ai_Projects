/**
 * Zero-dependency single-origin server for eCommerce-Application.
 *
 * - Serves the React production build (frontend/build) as static files.
 * - Falls back to index.html for client-side (SPA) routes.
 * - Proxies every /ecom/* request to the Spring Boot backend on port 9090.
 *
 * This means the whole app (UI + API) is reachable from ONE origin / ONE port,
 * so it can be exposed with a single tunnel.
 *
 * Usage:  node proxy-server.js
 * Env:    PORT (default 5050), BACKEND (default http://localhost:9090)
 */
...
const BACKEND = process.env.BACKEND || "http://localhost:9090";
...
const server = http.createServer((req, res) => {
  // Proxy all backend ecom requests
...
server.listen(PORT, () => {
  ...
  console.log(`Proxying /ecom -> ${BACKEND}`);
});
