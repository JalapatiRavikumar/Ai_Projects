...
const buildFallbackCoachReply = (message) => {
    const userText = String(message || "").trim();
    return (
        `I've got your back! Based on your message "${userText}", here is a quick action plan:\n\n` +
        `1) Pick one target role and list top 3 skills expected.\n` +
        `2) Practice a focused answer using STAR format (Situation, Task, Action, Result).\n` +
        `3) End with measurable impact from your project work.\n\n` +
        `Ready to practice now? Try our [HR Interview Room](/interview/hr/record) or review your resume in [ATS Checker](/resume/ats-check).`
    );
};
...
const chatWithCoach = async (req, res) => {
    try {
        ...
        if (!apiKey) {
            const fallbackReply = buildFallbackCoachReply(message);
            chat.messages.push({ role: "user", content: message });
            chat.messages.push({ role: "model", content: fallbackReply });
            await chat.save();
            return res.status(200).json({ reply: fallbackReply, history: chat.messages });
        }
...
`;
        ...
        if (isGeminiKeyError(error)) {
            ...
            let chat = await CoachMateChat.findOne({ userId: req.user._id });
            if (!chat) chat = new CoachMateChat({ userId: req.user._id, messages: [] });
