...
The whole app (React UI + Spring Boot API) is served from a **single origin** through one tunnel — a small zero-dependency Node proxy ([`deploy/proxy-server.js`](deploy/proxy-server.js)) serves the React build and forwards `/api` to the backend.

> ⚠️ **Note:** This is a temporary Cloudflare quick-tunnel pointing to a locally running instance. It is live only while the local servers and tunnel are running, and the random subdomain changes on every restart. To bring it back up after a restart, run [`deploy/start-live.ps1`](deploy/start-live.ps1) and use the new URL it prints. For a **permanent** URL, deploy to a host such as Render (backend + MySQL) and Vercel/Netlify (frontend) — see [`deploy/README.md`](deploy/README.md).
